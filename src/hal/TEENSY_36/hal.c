...
volatile DMAMEM int wake_source = UNKNOWN_WAKE;
...
void enter_vlpr(void)
{
    ...
    while ((PMC_REGSC & PMC_REGSC_REGONS))
        ; // 0 Regulator in __stop Reg mode
...
void pee_blpi(void)
{
    ...
    while (MCG_S & MCG_S_PLLST)
        ; // Wait for PLLST status bit to clear to
...
int llwu_disable(volatile llwu_mask_t *mask)
{
    ...
    int source = UNKNOWN_WAKE;
    if (flag & LLWU_PF1_WUF0)
        source = 26;
    else if (flag & LLWU_PF1_WUF3)
        source = 33;
    else if (flag & LLWU_PF1_WUF4)
        source = 4;
    else if (flag & LLWU_PF1_WUF5)
        source = 16;
    else if (flag & LLWU_PF1_WUF6)
        source = 22;
    else if (flag & LLWU_PF1_WUF7)
        source = 9;

    else if (flag & LLWU_PF1_WUF0)
        source = 59;
    else if (flag & LLWU_PF1_WUF1)
        source = 60;
    else if (flag & LLWU_PF1_WUF2)
        source = 62;

    else if ((flag >> 8) & LLWU_PF2_WUF8)
        source = 10;
    else if ((flag >> 8) & LLWU_PF2_WUF9)
        source = 13;
    else if ((flag >> 8) & LLWU_PF2_WUF10)
        source = 11;
    else if ((flag >> 8) & LLWU_PF2_WUF11)
        source = 30;
    else if ((flag >> 8) & LLWU_PF2_WUF12)
        source = 2;
    else if ((flag >> 8) & LLWU_PF2_WUF13)
        source = 7;
    else if ((flag >> 8) & LLWU_PF2_WUF14)
        source = 6;
    else if ((flag >> 8) & LLWU_PF2_WUF15)
        source = 21;

    else if ((flag >> 24) & LLWU_MF5_MWUF0)
        source = 36;
    else if ((flag >> 24) & LLWU_MF5_MWUF1)
        source = 34;
    else if ((flag >> 24) & LLWU_MF5_MWUF2)
        source = 34;
    else if ((flag >> 24) & LLWU_MF5_MWUF3)
        source = 34;
    else if ((flag >> 24) & LLWU_MF5_MWUF4)
        source = 37;
    else if ((flag >> 24) & LLWU_MF5_MWUF5)
        source = 35;
    LLWU_PE1 = 0;
    LLWU_PE2 = 0;
    LLWU_PE3 = 0;
...
{
    volatile llwu_mask_t *mask = &llwuMask;
    if (module & LLWU_LPTMR_MOD)
        mask->ME |= LLWU_ME_WUME0;
    else if (module & LLWU_RTCA_MOD)
        mask->ME |= LLWU_ME_WUME5;
    else if (module & LLWU_RTCS_MOD)
        mask->ME |= LLWU_ME_WUME7;
    else if (module & LLWU_TSI_MOD)
        mask->ME |= LLWU_ME_WUME4;
    else if (module & LLWU_CMP0_MOD)
        mask->ME |= LLWU_ME_WUME1;
    else if (module & LLWU_CMP1_MOD)
        mask->ME |= LLWU_ME_WUME2;
    else if (module & LLWU_CMP2_MOD)
        mask->ME |= LLWU_ME_WUME3;
}
//----------------------------------------------------------------------------------
void llwu_configure_pin_mask(uint8_t pin, uint8_t rise_fall)
{
    ...
    uint8_t detect_type;
    if (rise_fall == RISING)
        detect_type = LLWU_PIN_RISING;
    else if (rise_fall == FALLING)
        detect_type = LLWU_PIN_FALLING;
    else if (rise_fall == CHANGE)
        detect_type = LLWU_PIN_ANY;
    else
        detect_type = LLWU_PIN_ANY;

    if (pin == 2)
        mask->PE4 |= LLWU_PE4_WUPE12(detect_type);
    else if (pin == 4)
        mask->PE2 |= LLWU_PE2_WUPE4(detect_type);
    else if (pin == 6)
        mask->PE4 |= LLWU_PE4_WUPE14(detect_type);
    else if (pin == 7)
        mask->PE4 |= LLWU_PE4_WUPE13(detect_type);
    else if (pin == 9)
        mask->PE2 |= LLWU_PE2_WUPE7(detect_type);
    else if (pin == 10)
        mask->PE3 |= LLWU_PE3_WUPE8(detect_type);
    else if (pin == 11)
        mask->PE3 |= LLWU_PE3_WUPE10(detect_type);
    else if (pin == 13)
        mask->PE3 |= LLWU_PE3_WUPE9(detect_type);
    else if (pin == 16)
        mask->PE2 |= LLWU_PE2_WUPE5(detect_type);
    else if (pin == 21)
        mask->PE4 |= LLWU_PE4_WUPE15(detect_type);
    else if (pin == 22)
        mask->PE2 |= LLWU_PE2_WUPE6(detect_type);
    else if (pin == 26)
        mask->PE1 |= LLWU_PE1_WUPE0(detect_type);
    else if (pin == 30)
        mask->PE3 |= LLWU_PE3_WUPE11(detect_type);
    else if (pin == 33)
        mask->PE1 |= LLWU_PE1_WUPE3(detect_type);
    else if (pin == 59)
        mask->PE1 |= LLWU_PE1_WUPE0(detect_type);
    else if (pin == 60)
        mask->PE1 |= LLWU_PE1_WUPE1(detect_type);
    else if (pin == 62)
        mask->PE1 |= LLWU_PE1_WUPE2(detect_type);
}
//----------------------------------------------------------------------------------
void wakeup_isr(void)
{
    ...
    case IRQ_PORTA:
    {
        ...
        PORTA_ISFR = isfr_a;
        if (isfr_a & CORE_PIN3_BITMASK)
            source = 3;
        else if (isfr_a & CORE_PIN4_BITMASK)
            source = 4;
        else if (isfr_a & CORE_PIN25_BITMASK)
            source = 25;
        else if (isfr_a & CORE_PIN26_BITMASK)
            source = 26;
        else if (isfr_a & CORE_PIN27_BITMASK)
            source = 27;
        else if (isfr_a & CORE_PIN28_BITMASK)
            source = 28;
        else if (isfr_a & CORE_PIN39_BITMASK)
            source = 39;
        break;
    }
    case IRQ_PORTB:
    {
        ...
        PORTB_ISFR = isfr_b;
        if (isfr_b & CORE_PIN0_BITMASK)
            source = 0;
        else if (isfr_b & CORE_PIN1_BITMASK)
            source = 1;
        else if (isfr_b & CORE_PIN16_BITMASK)
            source = 16;
        else if (isfr_b & CORE_PIN17_BITMASK)
            source = 17;
        else if (isfr_b & CORE_PIN18_BITMASK)
            source = 18;
        else if (isfr_b & CORE_PIN19_BITMASK)
            source = 19;
        else if (isfr_b & CORE_PIN29_BITMASK)
            source = 29;
        else if (isfr_b & CORE_PIN30_BITMASK)
            source = 30;
        else if (isfr_b & CORE_PIN31_BITMASK)
            source = 31;
        else if (isfr_b & CORE_PIN32_BITMASK)
            source = 32;
    ...
    case IRQ_PORTC:
    {
        ...
        PORTC_ISFR = isfr_c;
        if (isfr_c & CORE_PIN9_BITMASK)
            source = 9;
        else if (isfr_c & CORE_PIN10_BITMASK)
            source = 10;
        else if (isfr_c & CORE_PIN11_BITMASK)
            source = 11;
        else if (isfr_c & CORE_PIN12_BITMASK)
            source = 12;
        else if (isfr_c & CORE_PIN13_BITMASK)
            source = 13;
        else if (isfr_c & CORE_PIN15_BITMASK)
            source = 15;
        else if (isfr_c & CORE_PIN22_BITMASK)
            source = 22;
        else if (isfr_c & CORE_PIN23_BITMASK)
            source = 23;
        else if (isfr_c & CORE_PIN35_BITMASK)
            source = 35;
        else if (isfr_c & CORE_PIN36_BITMASK)
            source = 36;
        else if (isfr_c & CORE_PIN37_BITMASK)
            source = 37;
        else if (isfr_c & CORE_PIN38_BITMASK)
            source = 38;
        break;
    }
    case IRQ_PORTD:
    {
        ...
        PORTD_ISFR = isfr_d;
        if (isfr_d & CORE_PIN2_BITMASK)
            source = 2;
        else if (isfr_d & CORE_PIN5_BITMASK)
            source = 5;
        else if (isfr_d & CORE_PIN6_BITMASK)
            source = 6;
        else if (isfr_d & CORE_PIN7_BITMASK)
            source = 7;
        else if (isfr_d & CORE_PIN8_BITMASK)
            source = 8;
        else if (isfr_d & CORE_PIN14_BITMASK)
            source = 14;
        else if (isfr_d & CORE_PIN20_BITMASK)
            source = 20;
        else if (isfr_d & CORE_PIN21_BITMASK)
            source = 21;
        break;
    }
    case IRQ_PORTE:
    {
        ...
        PORTE_ISFR = isfr_e;
        if (isfr_e & CORE_PIN24_BITMASK)
            source = 24;
        else if (isfr_e & CORE_PIN33_BITMASK)
            source = 33;
        else if (isfr_e & CORE_PIN34_BITMASK)
            source = 34;
        break;
    }
    case IRQ_LLWU:
